...
#include "nebula/net/handler/telegram/tcp_transport_codec.h"
...
static inline const char* ToString(TcpTransportDecoder::CodecState state) {
  switch (state) {
    case TcpTransportDecoder::CodecState::INVALID :
      return "INVALID";
      break;
    case TcpTransportDecoder::CodecState::WAIT_FIRST_PACKET :
      return "WAIT_FIRST_PACKET";
      break;
    case TcpTransportDecoder::CodecState::WAIT_PACKET_LENGTH_1 :
      return "WAIT_PACKET_LENGTH_1";
      break;
    case TcpTransportDecoder::CodecState::WAIT_PACKET_LENGTH_3 :
      return "WAIT_PACKET_LENGTH_3";
      break;
    case TcpTransportDecoder::CodecState::WAIT_PACKET :
      return "WAIT_PACKET";
      break;
    case TcpTransportDecoder::CodecState::ERROR :
      return "ERROR";
      break;
    default:
      break;
...
void TcpTransportDecoder::read(Context* ctx, folly::IOBufQueue& q) {
  if (UNLIKELY(state_ == TcpTransportDecoder::CodecState::INVALID ||
               state_ == TcpTransportDecoder::CodecState::ERROR)) {
...
std::pair<bool, bool> TcpTransportDecoder::decode(Context* ctx,
  ...
  do {
    if (state_ == CodecState::WAIT_FIRST_PACKET) {
      DCHECK(last_data_length_ == 64) << "decode - ";
...
/*
      auto rv = SetupAuthKey(folly::ByteRange(auth_key, 64));
      if (!rv) {
        LOG(ERROR) << "decode - setup auth_key error, key: " << ToHexStr(folly::ByteRange(auth_key, 64));
        ctx->fireReadException(folly::make_exception_wrapper<std::runtime_error>("decode - auth_key parser error!!!!"));
        // break;
        return std::make_pair(false, false);
      }
 */
      ...
      DCHECK(last_data_length_ == 1) << "decode - ";
...
bool TcpTransportDecoder::SetupAuthKey(folly::ByteRange auth_key) {
  try {
    // 提取key
...
folly::Future<folly::Unit> TcpTransportDecoder::write(Context* ctx, std::unique_ptr<folly::IOBuf> out) {
  // 长度
