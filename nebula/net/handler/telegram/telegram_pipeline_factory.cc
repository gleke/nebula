...
#include "nebula/net/handler/telegram/mtproto_transport_handler.h"
...
///////////////////////////////////////////////////////////////////////////////////////////
nebula::TelegramPipeline::Ptr TelegramPipelineFactory::newPipeline(std::shared_ptr<folly::AsyncTransportWrapper> sock) {
  auto pipeline = nebula::TelegramPipeline::create();
  ...
  pipeline->addBack(TelegramHandler(service_));
...
///////////////////////////////////////////////////////////////////////////////////////////
nebula::TelegramPipeline::Ptr TelegramClientPipelineFactory::newPipeline(std::shared_ptr<folly::AsyncTransportWrapper> sock) {
  auto pipeline = nebula::TelegramPipeline::create();
...
///////////////////////////////////////////////////////////////////////////////////////////
nebula::TelegramPipeline::Ptr TelegramServerPipelineFactory::newPipeline(std::shared_ptr<folly::AsyncTransportWrapper> sock) {
  auto pipeline = nebula::TelegramPipeline::create();
  pipeline->setReadBufferSettings(kDefaultMinAvailable, kDefaultAllocationSize);
  pipeline->addBack(wangle::AsyncSocketHandler(sock));
  pipeline->addBack(MTProtoTransportHandler());
