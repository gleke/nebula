...
#ifndef NUBULA_NET_ZPROTO_ZPROTO_API_MESSAGE_BOX_H_
#define	NUBULA_NET_ZPROTO_ZPROTO_API_MESSAGE_BOX_H_
...
inline RpcRequestPtr MakeRpcRequest(const google::protobuf::Message& message) {
...
//  可以直接生成EncodedRpcOk
// template<typename MESSAGE>
inline ProtoRpcResponsePtr MakeRpcOK(const google::protobuf::Message& message) {
  // static_assert(std::is_base_of<google::protobuf::Message, MESSAGE>::value,
  //              "MessaegType must be google::protobuf::Message");
...
inline bool ToRpcRequestMessage(const std::shared_ptr<RpcRequest>& request,
  ...
  int length = (int)(request->body->computeChainDataLength());
  char* data = new char[length];
  c.pull(data, length);
  // rv = message_.ParseFromArray(data, message_.ByteSize());
...
//  可以直接生成EncodedPush
// template<typename MESSAGE>
inline PushPtr MakePush(const google::protobuf::Message& message) {
  // static_assert(std::is_base_of<google::protobuf::Message, MESSAGE>::value,
  //              "MessaegType must be google::protobuf::Message");
  
  auto request = std::make_shared<Push>();
  return request;
}
