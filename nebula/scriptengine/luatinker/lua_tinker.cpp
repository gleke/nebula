...
#include "nebula/scriptengine/luatinker/lua_tinker.h"
...
// 	lua_pushstring(L, temp);
  ...
  std::string temp = folly::to<std::string>(*reinterpret_cast<const int64_t*>(lua_topointer(L, 1)));
  // base::Int64ToString(*reinterpret_cast<const int64*>(lua_topointer(L, 1)));
...
static int tostring_u64(lua_State *L)
{
  ...
  std::string temp = folly::to<std::string>(*reinterpret_cast<const uint64_t*>(lua_topointer(L, 1)));
  // base::Uint64ToString(*reinterpret_cast<const uint64*>(lua_topointer(L, 1)));
	lua_pushstring(L, temp.c_str());
	return 1;
}
...
template<>
folly::StringPiece lua_tinker::read(lua_State *L, int index)
{
  size_t s_len = 0;
  const char* s = lua_tolstring(L, index, &s_len);
  return folly::StringPiece(s, s_len);
}
...
template<>
void lua_tinker::push(lua_State *L, folly::StringPiece ret)
{
  lua_pushlstring(L, ret.data(), ret.size());
}

template<>
void lua_tinker::push(lua_State *L, const folly::StringPiece& ret)
{
  lua_pushlstring(L, ret.data(), ret.size());
}
