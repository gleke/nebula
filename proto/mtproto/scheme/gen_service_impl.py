...
output_header = output_path + '/service_impl.h'
output_source = output_path + '/service_impl.cc'
...
for restype in funcsList:
  ...
  for data in v:
    funcs.append([data[0], resType, data[8]]);
...
for v in funcs:
    ...
    # registers.append('  ZRpcUtil::Register(mtproto::TL_' + name + '::CLASS_ID, ' + ');\n');
    ...
    classTypesTexts += 'int ' + name + '(const mtproto::TL_' + name + '& request, std::shared_ptr<mtproto::' + resType + '>& response) override;\n\n';

    classFuncsTexts += 'int ' + name + '(const mtproto::TL_' + name + '& request, std::shared_ptr<mtproto::' + resType + '>& response) {\n';
    classFuncsTexts += '  LOG(INFO) << "' + name + ' - " << request.ToString();\n\n';
    classFuncsTexts += '  return kErrNotImpl;\n}\n\n';

header = '' + classTypesTexts + '\n\n';
source = '' + classFuncsTexts + '\n\n';
